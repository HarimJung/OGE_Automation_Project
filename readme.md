...
OGE_Automation_Project/,Root Folder,Contains all project assets and scripts.,N/A
├── data_input/,Folder,Container for ALL raw data and mapping files.,Add new OGE data here annually.
│   ├── OGE_Data_202X.csv,File,"The original raw OGE budget files (e.g., 2024, 2025).","New annual data (e.g., 2026.csv) is placed here."
│   ├── Policy_Tags_Mapping.csv,File,"Master lookup table for assigning PF4C, GEWE, and Climate tags.",Update this file only if policy criteria changes.
│   └── Language_Mapping_PT.csv,File,Lookup table containing Portuguese/English translations for the dashboard interface.,Rarely updated.
├── **setup_and_etl.py**,Script,The core Python automation script. Reads all inputs and produces the output.,DO NOT MODIFY.
├── **update.bat**,Batch File,"The ""One-Click"" Execution Tool. Runs the Python script seamlessly.",Double-click this file to perform the annual update.
└── **OGE_Master.hyper**,Database,"Final Output. Compressed, cleaned, and tagged data ready for Tableau connection.",Generated by the script. Connects to the TWBX dashboard.
